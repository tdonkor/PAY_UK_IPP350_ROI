...
using System.Timers;
using PAY_UK_IPP350_ROI.C3NetCommunicator;
using PAY_UK_IPP350_ROI.Communicator;

namespace PAY_UK_IPP350_ROI
{

    public class PayIngenicoIPP350_ROI : ICommunicatorCallbacks
    {
        public const string PAY_INGENICO_IPP350_ROI_LOG = "Pay_Ingenico_IPP350_ROI";
        ...
        public PayIngenicoIPP350_ROI(CoreCommunicator coreCommunicator)
        {
        ...
        {
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "call Initialize");
            ...
            if (IsCallbackMethodExecuting)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        another method is executing.");

                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall Initialize");
            ...
            if (!GetInitParameters(parameters.ToString()))
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        failed to deserialize the init parameters.");
            }
            ...
            else
            {
                ...
                if (!c3NetCommunicator.Send(requestString))
                {
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        failed to send initialization request.");
                ...
                if (!WaitingEFTBeginConfirmationResponse())
                {
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        No EFT Begin Confirmation was received");
                    IsCallbackMethodExecuting = false;
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall Initialize");
                    return;
                }
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        received EFT Begin Confirmation.");
                ...
                if (!WaitingEFTEndResponse())
                {
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        No EFT End Response was received");
                    IsCallbackMethodExecuting = false;
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall Initialize");
                    return;
                }
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        received EFT End Confirmation.");

                if (eftEndResponse.Error == 0)
                {
                    coreCommunicator.SendMessage(CommunicatorMethods.Init, new { Status = 0, Description = "Ready" });
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        EFT succesfully initialized");
                ...
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Error Code: {0}", eftEndResponse.Error);
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Error Code Response: {0}", eftEndResponse.ErrorResponseCode);
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Error Message:{0}", eftEndResponse.ErrorMessage);
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        EFT failed to initialized");
                }
            }
        ...
        {
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "call Pay");
            ...
            if (IsCallbackMethodExecuting)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall Pay");
            ...
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        deserialize the pay request parameters.");
            ...
            if (payRequest == null)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        failed to deserialize the pay request parameters.");
                return;
            }
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        deserialized pay request parameters.");
            ...
            if (payRequest.Amount == 0)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        amount can't be zero.");
                return;
            }
            ...
            if (string.IsNullOrEmpty(payRequest.TransactionReference))
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        transaction reference can't be empty.");
                return;
            }
            ...
            else
            {
                ...
                if (eftEndResponse.Error == 0)
                {
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Tender Media: {0}", eftEndResponse.TenderMedia);

                    coreCommunicator.SendMessage(CommunicatorMethods.Pay, new { Status = 0, Description = "Successful payment", PayDetails = payDetails });
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        credit card payment succeeded.");
                }
                else
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Error Code: {0}", eftEndResponse.Error);
                    ...
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        credit card payment failed.");
                }
            }
        ...
        private void SaveTicket(string ticketContent, bool isMerchantTicket)
        {
            ...
            {
                Log.Error(PAY_INGENICO_IPP350_ROI_LOG, string.Format("        failed to save ticket.\r\n{0}", ex.ToString()));
            }
        }
        ...
        public void Test(object parameters)
        {
            ...
            if (IsCallbackMethodExecuting)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        another method is executing (Stopped Test Method execution).");
            ...
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "call Test");

            if (IsTerminalReady)
            {
                coreCommunicator.SendMessage(CommunicatorMethods.Test, new { Status = 0 });
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        success.");
            }
            else
            {
                coreCommunicator.SendMessage(CommunicatorMethods.Test, new { Status = -1 });
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        failed.");
            }

            IsCallbackMethodExecuting = false;

            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall Test");
        }
        ...
        {
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        checking payment status");
            ...
            if (!ConnectToC3Net())
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        the connection is down.");
            }
            else
            {
        ...
        public void ElapsedPeriodicalCheckTimer(object sender, ElapsedEventArgs args)
        {
            try
            {
                ...
                if (IsCallbackMethodExecuting)
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        periodical check postponed");
                }
                else
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "call PeriodicalCheck");
                    IsCallbackMethodExecuting = true;
                    CheckPayment();
                    IsCallbackMethodExecuting = false;
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall PeriodicalCheck");
                }
            }
            catch (Exception ex)
            {
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, string.Format("        ElapsedPeriodicalCheckTimer: {0}", ex.ToString()));
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "endcall PeriodicalCheck");
            }
        }
        ...
        private PayRequest GetPayRequest(string payRequestJSonString)
        {
            ...
            {
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, string.Format("        GetPayRequest: {0}", ex.ToString()));
            }
        ...
        private void DoOnReceivedMessageEventHandler(object sender, ReceivedMessageEventArgs args)
        {
            ...
            else if (args.Message.IndexOf(C3MessageTypes.POS_DISPLAY) == 0)
            {
                ...
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [DISPLAY POS MESSAGE] {0} [RIC]", eFTPOSDispplayMessage.Description);

                }
                //RESPOND_WITH_1SEC_DELAY
                else if (eFTPOSDispplayMessage.ActionType == EFTPOSDisplayMessageActionTypes.RESPOND_WITH_1SEC_DELAY)
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [DISPLAY POS MESSAGE] {0} [RWD]", eFTPOSDispplayMessage.Description);
                ...
                {
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [DISPLAY POS MESSAGE] {0} [AFC]", eFTPOSDispplayMessage.Description);
                }
            ...
            else if (args.Message.IndexOf(C3MessageTypes.PROGRESS_MESSAGE) == 0)
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [PROGRESS MESSAGE]");
        ...
        private void SendProgressMessage(string message)
        {
            ...
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Sending Progress Message to Core");
        ...
        private void SendPosDisplayResponse(string message)
        {
            if (!c3NetCommunicator.Send(message))
            {
                ...
                if (c3NetCommunicator.Send(message))
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [DISPLAY POS MESSAGE] Send receival confirmation (second attept)");
            }
            else
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [DISPLAY POS MESSAGE] Send receival confirmation");
        }
        ...
        private void SendProgressMessageResponse(string message)
        {
            if (!c3NetCommunicator.Send(message))
            {
                //Make another attempt to sent the Display message confirmation 
                if (c3NetCommunicator.Send(message))
                    Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [PROGRESS MESSAGE] Send receival confirmation (second attept)");
            }
            else
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        [PROGRESS MESSAGE] Send receival confirmation");
        }
        ...
        private bool ConnectToC3Net()
        {
            ...
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        trying to establish the connection to the C3Net.");
            ...
            {
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        connection to the C3Net is down.");
                return false;
            }
            else
            {
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        connection to the C3Net was established.");
                return true;
            }
        }
        ...
        private bool GetInitParameters(string initJson)
        {
            try
            {
                ...
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Currency: {0}", CurrencyValue);

                TerminalID = jObject["TerminalID"].ToString();
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Terminal ID: {0}", TerminalID);


                TestTimeout = Convert.ToInt32(jObject["TestTimeout"].ToString()) * 60 * 1000;
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        Test Timeout: {0}", TestTimeout);
            ...
            {
                Log.Info(PAY_INGENICO_IPP350_ROI_LOG, string.Format("        GetInitParameters: {0}", ex.ToString()));
            }
            return false;
        }
        ...
        {
            //Log the message 
            Log.Info(PAY_INGENICO_IPP350_ROI_LOG, "        {0}", message);
